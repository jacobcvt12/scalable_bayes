...
>- For any model $p(z, x)$ regardless of conjugacy of priors $p(z)$ to likelihood $p(z, x)$, we would like to approximate the posterior with variational inference
...
>- Calculate log joint density ($\log p(x, z_i) = \log p(x | z_i) + \log p(z_i)$)
