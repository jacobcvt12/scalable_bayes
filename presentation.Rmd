---
title: "Scalable Bayesian Methods"
subtitle: "Posterior Approximation in high dimension with large datasets"
author: "Jacob Carey"
date: \today
output: 
  beamer_presentation:
...
>- Your model takes too long to converge
>- You're impatient
>- Your model takes too long to converge

## Computational Complexity

- Iterations per parameter and per observation
- Also important, but not necessarily part of the computational complexity: number of total iterations
- Number of total iterations tends to increase the more parameters a model has, especially using Gibbs and MH
- Joint density (likelihood $\times$ prior) evaluations are fixed at number of parameters $m$ $\times$ number of observations $n$
...
- EM is a optimization based inference method
...
![Blei et al 2016](comparison.png)
...
Priors: $$\mu \sim \text{Normal}(\mu_\mu, \tau_\mu)$$
...
ggplot(results, aes(variable, value)) + 
    geom_point(aes(colour=covariate, shape=covariate)) + 
    geom_linerange(aes(ymin=lb, ymax=ub), linetype=2, color="black") +
    labs(x="Covariate",
         y="Value",
         title="Comparison of M-H and Variational Inference",
         colour="Method",
         shape="Method")
```
...
>- Optimize with respect to other divergences
>- Improve optimizations while retaining effiencies
>- Combining MCMC and VA/when to use one or the other
>- Statistical properties of VA
